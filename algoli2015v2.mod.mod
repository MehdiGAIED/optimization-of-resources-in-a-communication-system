...
//float d[i in 0..n-1]=min(D[i]);// distances minimales
...
for   (var i = 1; i <= 4; i++)
{
      ...
      accesspt aplifi=<i,<3.75*i,3.75*j,0>,1> ;// les ap sont en grille
...
//model 
         ...
         maximize 
               sum(u in 0..n-1) (sum(ap in 0..nap-1) (sum(t in temps ) y[item(AP,i)][item(users,j)][t]*log(debit_phy[u][ap]*(t/T))));//debit_atteint=debit_phy.t/T
...
//le temps d'allocation d'un AP lifi ne peut pas dépasser l'intervalle de TDMA T ***cette contrainte doit etre valable pour tous les APs lifi
 forall(ap in 1..napl)
     ...
     (sum(u in 1..n) sum(t in temps)y[item(APlifi,ap)][item(users,u)][t]*(t/T))<=1;
...
forall(ap in 1..napw)
  (sum(u in 1..n) sum(t in temps) y[item(APwifi,ap)][item(users,u)][t]*(t/T))<=Pdl;
